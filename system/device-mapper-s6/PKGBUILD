...
pkgname=device-mapper-s6
pkgver=20260512
pkgrel=1
pkgdesc="s6-rc service scripts for device-mapper"
arch=('any')
url="https://gitea.artixlinux.org/artix/s6-services"
groups=('s6-system')
provides=('init-device-mapper')
conflicts=('init-device-mapper')
depends=('device-mapper' 's6-base')
makedepends=('git')
backup=('etc/s6/config/device-mapper.conf')
...
package() {
    ...
    sh install.sh "device-mapper" "${pkgdir}"
}
