...
pkgname=(
  ...
  qemu-block-{curl,dmg,iscsi,nfs,ssh}
  ...
  qemu-img
  qemu-pr-helper
  qemu-system-{aarch64,alpha,arm,avr,hexagon,hppa,loongarch64,m68k,microblaze,mips,or1k,ppc,riscv,rx,s390x,sh4,sparc,tricore,x86,xtensa}
...
makedepends=(
  ...
  vte3
  zlib
  zlib-static  # for qemu-user-static
  zstd
)
source=(
...
sha512sums=('f30b36800b44ac0370e63341a8c5591019648a78d5929e18b0177873c0b6e4a0cbbddc069337cb06a3b5d20d15779c9b9786d0a92073c421fb07ae14a67d4646'
            'SKIP'
            ...
            '87c3175aeecbb84852f9aea91fd775af13c21340eef8047520d78550ef72d3d1935a396bb843fc571db8fcb5ca2154d8a135dcca04ca0dfeba3191b16a5a402c'
...
b2sums=('86f3094c0e43af8ccb822c79cb17bd1c15450c261164a7b0e106cec8fe057f5d7159a0bc163d63d013f27963bd698d05c34465fb7a2b906f48b8c49d01d9bc44'
        'SKIP'
        ...
        '3c8e365b9dbba8ca06e8be0b99f2e6393a8ad0019a3c6fddc2d7806b454fafd65e99a0a1dbedf55b86e982c2e7b2825e67f4d28a61d8078795cae499956e98f6'
...
_qemu_desktop_optdepends=(
  ...
  'qemu-system-hexagon: for Hexagon system emulator'
...
package_qemu-common() {
  ...
  (
    ...
    for _name in qemu-system-{aarch64,alpha,arm,avr,hexagon,hppa,i386,loongarch64,m68k,microblaze,mips{,64,64el,el},or1k,ppc{,64},riscv{32,64},rx,s390x,sh4{,eb},sparc{,64},tricore,x86_64,xtensa{,eb}}; do
  ...
  (
    ...
    _pick qemu-system-hexagon usr/bin/qemu-system-hexagon
    _pick qemu-system-hexagon usr/share/man/man1/qemu-system-hexagon.1*
    ...
    _pick qemu-system-microblaze usr/bin/qemu-system-microblaze
    _pick qemu-system-microblaze usr/share/man/man1/qemu-system-microblaze.1*

    _pick qemu-system-microblaze-firmware usr/share/qemu/dtb/petalogix-*.dtb
    ...
    _pick qemu-system-ppc-firmware usr/share/qemu/dtb/{bamboo,canyonlands,pegasos{1,2}}.dtb
    ...
    _pick qemu-system-ppc-firmware usr/share/qemu/u-boot-sam460.bin
    ...
    if [[ -e usr/bin/qemu-vmsr-helper ]]; then
      _pick qemu-vmsr-helper usr/bin/qemu-vmsr-helper
    fi
  )

  # remove files provided by edk2-{aarch64,ovmf,riscv}
...
package_qemu-audio-pipewire() {
  ...
  depends=(glibc libpipewire libpipewire-0.3.so qemu-common=$pkgver-$pkgrel)
  mv -v $pkgname/* "$pkgdir"
  _install_licenses
}
...
package_qemu-system-hexagon() {
  pkgdesc="QEMU system emulator for Hexagon"
...
package_qemu-ui-egl-headless() {
  ...
  depends=(glibc pixman libpixman-1.so qemu-common=$pkgver-$pkgrel qemu-ui-opengl=$pkgver-$pkgrel)
  mv -v $pkgname/* "$pkgdir"
  _install_licenses
}
...
package_qemu-ui-sdl() {
  ...
  depends=(glib2 libglib-2.0.so glibc libx11 pixman libepoxy libpixman-1.so qemu-common=$pkgver-$pkgrel sdl2_image sdl2)
  mv -v $pkgname/* "$pkgdir"
  _install_licenses
}
...
package_qemu-base() {
  ...
  optdepends_x86_64=('qemu-vmsr-helper: for accessing the Running Average Power Limit (RAPL) MSR')
...
package_qemu-emulators-full() {
  ...
  depends=(
    qemu-system-{aarch64,alpha,arm,avr,hexagon,hppa,loongarch64,m68k,microblaze,mips,or1k,ppc,riscv,rx,s390x,sh4,sparc,tricore,x86,xtensa}=$pkgver-$pkgrel
...
package_qemu-full() {
  ...
  depends=(
    ...
    qemu-block-iscsi=$pkgver-$pkgrel
  ...
  depends_x86_64=(qemu-vmsr-helper=$pkgver-$pkgrel)
