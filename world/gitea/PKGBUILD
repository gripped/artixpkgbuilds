# Maintainer: Bruno Pagani <archange@archlinux.org>
# Maintainer: Robin Candau <antiz@archlinux.org>
# Contributor: Maxime Gauduin <alucryd@archlinux.org>
# Contributor: Frederik Schwan <frederik dot schwan at linux dot com>

pkgname=gitea
pkgver=1.27.3
pkgrel=1
pkgdesc="Painless self-hosted Git service, community managed."
url="https://gitea.io"
arch=('x86_64')
license=('MIT')
depends=('git' 'zstd')
makedepends=('go' 'nodejs' 'npm' 'pnpm')
optdepends=('mariadb: MariaDB support'
            'memcached: MemCached support'
            'openssh: GIT over SSH support'
            'pam: Authentication via PAM support'
            'postgresql: PostgreSQL support'
            'valkey: Valkey support'
            'sqlite: SQLite support'
            'gitea-runner: Official runner for Gitea'
            'tea: A command line tool to interact with Gitea servers')
checkdepends=('openssh')
options=('!lto')
backup=("etc/${pkgname}/app.ini")
source=("git+https://github.com/go-gitea/gitea.git#tag=v${pkgver}?signed"
        "${pkgname}.tmpfiles"
        "${pkgname}.sysusers"
        'tests_hash_parsing_and_go_1.27_compat.patch')
sha256sums=('8d55a028f6bbfeb6da0945acb0d2146a29d73f32df3f76dac70e2883aee7b510'
            '1521fd7edc3830c695698ffe9835709f1408040b5ec989f07410972c894fa8ba'
            'e29dcc74b2f52e32d3931103d03f202d1cafe18e7eb9a271da0f50ef61dc8446'
            'ce7d824a53d177f04695f50468989ce733986fcc26ac93fe57e7db6d8dc7139e')
validpgpkeys=(B56E3C7437A49E136862F5DE9D8A57ADAA232E95  # Matti Ranta <matti@mdranta.net>, retrieved from https://github.com/techknowlogick.gpg
              D8F9672D77C0BB60A024C23EDFDE60A0093EB926  # Lauris Bukšis-Haberkorns <lauris@nix.lv>, retrieved from https://github.com/lafriks.gpg
              BA66F67FD73F7058D712D308C3B7C91B632F738A  # Lunny Xiao <xiaolunwen@gmail.com>, retrieved from https://github.com/lunny.gpg
              EBDC78572A6B438A12B9A913B4CAE843BC6C1375  # 'Zettat123' <zettat123@gmail.com>, retrieved from https://github.com/Zettat123.gpg
              8722B61D72341082553B201CB8BE6D610E61C862  # '6543' <6543@obermui.de>, retrieved from https://github.com/6543.gpg
              D2CF76DA95F201E9901532AB3CDE74631F13A748  # Andrew Thornton <art27@cantab.net>, retrieved from https://github.com/zeripath.gpg
              82A110A44DF1A28D50C093BFB853ADA5DA7BBF7A  # jolheiser <john@jolheiser.com>, retrieved from https://github.com/jolheiser.gpg
              FE7C3EAEB8CD8290390B12AD3DECE05F6D9A647C) # delvh <dev.lh@web.de>, retrieved from https://github.com/delvh.gpg

prepare() {
	cd "${pkgname}"

	# Fix some hash parsing and Go 1.27 compatibility issue with some tests
	patch -Np1 -i "${srcdir}/tests_hash_parsing_and_go_1.27_compat.patch"
	rm -v modules/json/jsonv1.go modules/json/jsonlegacy.go

	make deps-frontend deps-backend deps-tools
}

build() {
	cd "${pkgname}"
	export CGO_CPPFLAGS="${CPPFLAGS}"
	export CGO_CFLAGS="${CFLAGS}"
	export CGO_CXXFLAGS="${CXXFLAGS}"
	export CGO_LDFLAGS="${LDFLAGS}"
	export EXTRA_GOFLAGS="-buildmode=pie -mod=readonly -modcacherw"
	export LDFLAGS="-linkmode=external -compressdwarf=false -X 'gitea.dev/modules/setting.AppWorkPath=/var/lib/gitea/' -X 'gitea.dev/modules/setting.CustomConf=/etc/gitea/app.ini'"
	export TAGS="bindata sqlite sqlite_unlock_notify pam external_libzstd"
	make -j1
}

check() {
	cd "${pkgname}"
	# Avoid `error: could not lock config file /dev/null: Permission denied` from git
	export GIT_CONFIG_GLOBAL="${PWD}/gitconfig"
	# This test expects .gitconfig path to be "$HOME/.gitconfig" (which isn't the case due to the above workaround)
	rm -v modules/git/config_test.go
	# `-p 1` to prevent race conditions from go tests when trying to lock the gitconfig file
	make test-frontend
	make test-backend GOTEST_FLAGS="-p 1"
}

package() {
	cd "${pkgname}"
	install -Dm 755 "${pkgname}" "${pkgdir}/usr/bin/${pkgname}"
	install -Dm 644 LICENSE "${pkgdir}/usr/share/licenses/${pkgname}/LICENSE"
	install -Dm 644 custom/conf/app.example.ini "${pkgdir}/etc/${pkgname}/app.ini"
	install -Dm 644 "${srcdir}/${pkgname}.tmpfiles" "${pkgdir}/usr/lib/tmpfiles.d/${pkgname}.conf"
	install -Dm 644 "${srcdir}/${pkgname}.sysusers" "${pkgdir}/usr/lib/sysusers.d/${pkgname}.conf"
}
