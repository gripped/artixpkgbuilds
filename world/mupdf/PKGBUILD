...
prepare() {
	...
	# Allow building against the system cmark-gfm package instead of the vendored thirdparty dependency
	# (wires up the upstream USE_SYSTEM_CMARK_GFM path, which is incomplete in 1.28)
	# Patch taken from OpenSuse at https://build.opensuse.org/requests/1362065/changes
	patch -Np1 -i "${srcdir}/${pkgbase}-1.28.0-system-cmark-gfm.patch"
	# Allow building against recent clang
	patch -Np1 -i "${srcdir}/${pkgbase}-1.28.0-fix-build-with-recent-clang.patch"
	# Let the auto-created venv use system site-packages
	patch -Np1 -i "${srcdir}/${pkgbase}-1.28.3-autovenv-system-site-packages.patch"

	# Init submodules and disable the ones we don't need, we want our system libraries instead
	git submodule init
	rm -rvf thirdparty/{brotli,cmark-gfm,curl,freeglut,freetype,gumbo-parser,harfbuzz,jbig2dec,leptonica,libjpeg,openjpeg,tesseract,zint,zlib,zxing-cpp}
	git submodule deinit thirdparty/brotli
	git submodule deinit thirdparty/cmark-gfm
...

	{
	  ...
	  printf "USE_SYSTEM_CMARK_GFM := yes\n"
...
package_libmupdf() {
	...
	depends=('brotli' 'cmark-gfm' 'freetype2' 'glibc' 'gumbo-parser' 'harfbuzz' 'jbig2dec' 'leptonica' 'libgcc' 'libstdc++' 'libjpeg-turbo' 'openjpeg2' 'tesseract' 'zlib')
...
package_python-mupdf() {
	...
	depends=('glibc' 'libgcc' 'libstdc++' 'libmupdf' 'python')
