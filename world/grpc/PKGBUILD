...
source=(
    "$pkgbase::git+https://github.com/grpc/grpc.git#tag=v$pkgver"
    "${pkgname}-grpc-proto::git+https://github.com/grpc/grpc-proto.git"
...
prepare() {
  ...
  git config submodule."third_party/grpc-proto".url "${srcdir}/${pkgname}"-grpc-proto
  git -c protocol.file.allow=always submodule update --init -- third_party/grpc-proto third_party/googletest

  patch -p1 -i "$srcdir/fix-python-system-abseil.patch"
...
build() {
  ...
  if [[ "$(pacman -Q protobuf | grep -Po '[0-9.]+(?=-)')" != "$_protobuf_ver" ]]; then
  ...
  # PHP
  cd "$srcdir/$pkgbase/src/php/ext/$pkgbase"
  phpize
  ./configure --enable-grpc="$srcdir/$pkgbase"
  make

  # PHP Legacy
  cd "$srcdir/$pkgbase/src/php-legacy/ext/$pkgbase"
  phpize-legacy
  ./configure --enable-grpc="$srcdir/$pkgbase"
  make
...
package_grpc() {
  depends=('glibc' 'libgcc' 'zlib' 'c-ares' 'protobuf' 'openssl' 're2' 'abseil-cpp')
  provides=(
    'libupb.so'
...
package_python-grpcio-tools() {
  ...
  depends=('glibc' 'libgcc' 'libstdc++' 'protobuf' 'python' 'python-setuptools' 'python-protobuf' 'python-typing_extensions')
