...
depends=('curl' 'libcurl.so' 'libmpdclient' 'libmpdclient.so' 'fftw' 'libfftw3.so' 'boost-libs'
         ...
         'glibc' 'libgcc' 'libstdc++' 'ncurses' 'libncursesw.so' 'readline' 'libreadline.so' 'taglib')
...
prepare() {
  cd ${pkgname}
  git cherry-pick -n 2b4bd89e2e80a7496aa03b5e18b3f986425a17e4 # Fix build with boost 1.89
  autoreconf -fiv
