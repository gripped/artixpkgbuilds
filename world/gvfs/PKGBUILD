...
source=(
  ...
  0002-monitor-Do-not-load-monitors-when-GIO_USE_VFS-local.patch
)
...
prepare() {
  ...
  # https://gitlab.gnome.org/GNOME/gvfs/-/merge_requests/259
  git apply -3 ../0001-nfs-Support-libnfs-6-backport-to-1.56.patch

  # https://gitlab.gnome.org/GNOME/gvfs/-/merge_requests/193
  git apply -3 ../0002-monitor-Do-not-load-monitors-when-GIO_USE_VFS-local.patch
}
